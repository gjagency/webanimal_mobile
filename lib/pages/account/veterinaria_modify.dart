// lib/pages/mi_veterinaria_modify.dart
import 'dart:convert';
import 'dart:io';
...
class MiVeterinariaModify extends StatefulWidget {
  final String id;
  const MiVeterinariaModify({super.key, required this.id});

  @override
  State<MiVeterinariaModify> createState() => _MiVeterinariaModifyState();
}

class _MiVeterinariaModifyState extends State<MiVeterinariaModify> {
  ...
  Future<void> _loadVeterinaria() async {
    try {
      final vet = await MisVeterinariasService.getOne(widget.id);
      setState(() {
        _veterinaria = vet;
        ...
        _stateController.text = vet.location.state;
        _countryController.text = vet.location.country;
      ...
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('Error al cargar veterinaria')));
        context.pop();
      }
    }
  }
  ...
  Future<void> _saveVeterinaria() async {
    ...
    try {
      ...
      await MisVeterinariasService.modify(
        id: widget.id,
  ...
  Widget build(BuildContext context) {
    if (_isLoadingData) {
      return Scaffold(
        ...
        appBar: AppBar(title: Text('Editar Veterinaria')),
            ...
            GestureDetector(
              ...
              child: Container(
                ...
                child: _selectedImageFile != null
                    ...
                    : _veterinaria?.imageUrl != null
                    ? ClipRRect(
                        borderRadius: BorderRadius.circular(18),
                        child: Image.network(
                          _veterinaria!.imageUrl!,
                          fit: BoxFit.cover,
                        ),
                      )
                    : Center(
